...
rospy.loginfo('Lidar segmentation screen initializing...')

pc_pub = rospy.Publisher("/pc2_colored_screen", PointCloud2, queue_size=1)
...
def pointcloud_callback(point_cloud_msg):
    ...
    print(point_generator is not None)
    ...
    for point in point_generator:
        if not math.isnan(point[2]):
            ...
            if point[2] <= 0.30 and math.sqrt(point[0]**2 + point[1]**2 <= 8):
                r, g, b, a = 0, 255, 0, 255
                #modified_point = (point[0], point[1], point[2], 0xffff00, point[4])
            elif point[2] <= 0.4:
...
rospy.Subscriber("/points_for_save_screen", PointCloud2, pointcloud_callback, queue_size=1)
rospy.Subscriber("/detection/boxs", MarkerArray, marker_callback, queue_size=1)

rospy.loginfo('Lidar segmentation screen initialized')